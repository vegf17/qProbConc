...
sample :: ((C,LMem),Int) -> ProbT IO LMem
sample ((c,s),0) = ProbT $ return [(s, 0)] --From the original semantics
sample ((c,s),n) = do
...
runSample c s n = do
  memList <- runProbT $ sample ((c,s),n) --the probabilities associated to the configurations in the distribution
